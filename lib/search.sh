...
OMA_DEFAULT_CONFIG="$HOME/.local/share/omarchy/config"
...
OMA_SEARCH_EXTRA=(
  ...
  "waybar_menu|Waybar › Status bar visibility"
  "waybar_menu|Waybar › Modules"
  "waybar_menu|Waybar › Bar position"
  "waybar_menu|Waybar › Bar height"
  "waybar_menu|Waybar › Module spacing"
  "waybar_menu|Waybar › style.css"
...
_oma_changed_files() {
  local f rel default
  for f in "$OMA_HYPR_DIR"/*.conf "$HOME/.config/waybar/config.jsonc" \
    "$HOME/.config/waybar/style.css" "$HOME/.config/alacritty/alacritty.toml"; do
    [[ -f $f ]] || continue
    rel="${f#"$HOME"/.config/}"
